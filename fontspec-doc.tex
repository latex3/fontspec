...
The current default package option, \texttt{euenc}, selects the \texttt{EU1} or \texttt{EU2} encoding (\XeTeX/\LuaTeX, resp.) and loads the \pkg{xunicode} package.

A new package option, \texttt{tuenc}, switches the default \textsc{nfss} font encoding to \texttt{TU}.
...
Later in the year, the default will be changed to load \texttt{tuenc}.
Package authors and users who have referred explicitly to the encoding names \texttt{EU1} or \texttt{EU2} will need to update their documents in advance.
(See internal variable names described in \vref{sec:api} for how to do this properly.)
If anything goes wrong, \pkg{fontspec} can be loaded with the \texttt{euenc} option to revert to the 2016-and-previous behaviour.

\emph{While \pkg{fontspec} is providing the \texttt{TU} encoding, its interface should not be considered experimental; feedback welcome. Once \texttt{TU} is incorporated into the \LaTeXe\ kernel directly (hopefully for \TeX{}Live 2017), it will be considered stable.}
