...
A new package option, \texttt{ucenc}, which is selected by default, switches the \textsc{nfss} font encoding to \texttt{UC}.
\texttt{UC} is a new Unicode font encoding, intended for both \XeTeX\ and \LuaTeX\ engines, and automatically contains support for symbols covered by \LaTeX's traditional \texttt{T1} and \texttt{TS1} font encodings (for example, |\%|, |\textbullet|, |\"u|, and so on).
As a result, with this package option, Ross Moore's \pkg{xunicode} package is \textbf{not} loaded.

The old package behaviour can be achieved by loading the \texttt{euenc} package option. This selects the \texttt{EU1} or \texttt{EU2} encoding (\XeTeX/\LuaTeX, resp.) and loads the \pkg{xunicode} package.
...
\emph{While \pkg{fontspec} is providing the \texttt{UC} encoding, its interface should not be considered experimental; feedback welcome. Once \texttt{UC} is incorporated into the \LaTeXe\ kernel directly (later in 2016), it will be considered stable.}
...
If you find that \pkg{fontspec} is incorrectly changing the maths font when it shouldn't be, apply the |no-math| package option to manually
...
Since this has changed from \texttt{EU1}/\texttt{EU2} to \texttt{UC}, it is best to use the variables \cs{g_fontspec_encoding_tl} or \cs{UTFencname} instead.
...
that don't resolve well; better results can be achieved by using specific Unicode characters or proper LICRs with a Unicode font encoding.
