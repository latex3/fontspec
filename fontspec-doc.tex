...
\paragraph{\pkg{xunicode} and font encodings}
...
While the defaults have not yet changed, a new package option, \texttt{tuenc}, switches the default \textsc{nfss} font encoding to \texttt{TU}.
...
Later in the year, the default will be changed to load \texttt{tuenc} instead.
...
(See internal variable names described in \vref{sec:api}.)
...
Note that if you do so, \pkg{xunicode} will be automatically loaded for backwards compatibility.
...
\paragraph{\pkg{babel}}
\emph{The \pkg{babel} package is supported for certain languages.}
